...
<imx-data-table [dst]="dst" mode="auto"> </imx-data-table>
...
export class SelectIdentityComponent implements OnInit {
  ...
  constructor(private readonly qerApiClient: QerApiService) {
    ...
    this.displayedColumns = [this.schema.Columns[DisplayColumns.DISPLAY_PROPERTYNAME], this.schema.Columns.DefaultEmailAddress];
  }
...
<imx-data-table [dst]="dst" mode="manual">
  ...
  <imx-data-table-generic-column columnName="viewDetailsButton">
    <ng-template>
      <button mat-button color="primary">{{ '#LDS#Details' | translate }}</button>
    </ng-template>
