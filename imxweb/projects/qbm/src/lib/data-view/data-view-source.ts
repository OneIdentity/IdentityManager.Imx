...
export class DataViewSource<T extends TypedEntity = TypedEntity, ExtendedType = any> implements DataSource<T>, OnDestroy {
  ...
  // If the item is defined, then use the queue to find if its in the queue or failed. Otherwise return true
  private itemNotInQueueOrFailed = (item?: TypedEntity) =>
    item
      ? [QueuedActionState.Failed, QueuedActionState.NotInQueue].includes(
  ...
  public itemStatus: DataSourceItemStatus = {
    ...
    enabled: this.itemNotInQueueOrFailed,
    // By default, enabled = rowEnabled
    rowEnabled: this.itemNotInQueueOrFailed,
  };
