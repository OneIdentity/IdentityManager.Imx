...
@Component({
  ...
  styleUrls: ['./edit-number-range.component.scss'],
})
export class EditNumberRangeComponent implements CdrEditor, OnDestroy {
  ...
  private setupValidators(): void {
    // Validators for individual input controls: validate their own values only
    ...
    inputValidators.push(this.checkIntegerValue()); // validate integer + Min/Max constraints

    this.valueFrom.setValidators(inputValidators);
    this.valueTo.setValidators(inputValidators);

    // Validator for the main control: validate range order relationship
    this.control.setValidators([this.checkRangeOrder()]);
  }
