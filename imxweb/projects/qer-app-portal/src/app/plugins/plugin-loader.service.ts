...
import { imx_SessionService, ClassloggerService, AppConfig } from 'qbm';
        ...
        for (const plugin of this.appInfo.PlugIns) {
          ...
          try {
            if (isDevMode()) {
              ...
              const config = (await this.httpClient.get('appconfig.json').toPromise()) as AppConfig;
              this.logger.debug(this, '▶️  Config. PROD mode.', config);
              container =
                await this.importProdMode(`${host}${config.Basepath}/html/${plugin.Container}/bundles/${plugin.Container}.umd.js`);
            }
            ...
            this.plugins.push(elementModuleRef.instance as ModulePlugin);
          } catch (e) {
