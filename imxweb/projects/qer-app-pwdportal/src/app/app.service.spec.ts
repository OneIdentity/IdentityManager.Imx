...
describe('AppService', () => {
  ...
  it('calls translation provider init on session response', async () => {
    await AppService.init(service)();
    mocks.authentication.onSessionResponse.next({});
    expect(mocks.translationProvider.init).toHaveBeenCalled();
  });

  it('calls translation provider init on session response - with null check', async () => {
    await AppService.init(service)();
    mocks.authentication.onSessionResponse.next(undefined);
    expect(mocks.translationProvider.init).toHaveBeenCalled();
  });
});
