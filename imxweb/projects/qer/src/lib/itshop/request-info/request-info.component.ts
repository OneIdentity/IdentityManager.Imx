...
export class RequestInfoComponent implements OnInit {
  ...
  public async ngOnInit(): Promise<void> {
    ...
    try {
      this.propertyInfo = this.request == null || this.request.propertyInfo == null ? []
        : this.request.propertyInfo.filter(elem => this.isForView(elem));
      ...
      this.approverContainer = new ApproverContainer(
        {
          ...
          approvers: (await this.itshopService.getApprovers(this.request.GetEntity().GetKeys()[0]))
            .Data.map(elem => elem.UID_Person.value)
        },
        (await this.projectConfig.getConfig()).ITShopConfig,
        this.logger
      );
