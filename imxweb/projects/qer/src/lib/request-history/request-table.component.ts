...
import {
  DataSourceToolbarSettings, DataSourceToolbarFilter, imx_SessionService, ImxTranslationProviderService, DataTableComponent, SettingsService
} from 'qbm';
...
export class RequestTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public get canWithdrawAdditionalApprover(): boolean {
    return this.itShopConfig && this.itShopConfig.VI_ITShop_OrderHistory_CancelOrder &&
  ...
  public get canRecallLastQuestion(): boolean { return this.selectedItems.every(item => item.canRecallLastQuestion); }
  public get canRevokeHoldStatus(): boolean { return this.selectedItems.every(item => item.canRevokeHoldStatus); }
  public get canProlongateRequest(): boolean { return this.selectedItems.every(item => item.canProlongate); }
  public get canWithdrawRequest(): boolean { return this.selectedItems.every(item => item.CancelRequestAllowed.value); }
  public get canUnsubscribeRequest(): boolean { return this.selectedItems.every(item => item.UnsubscribeRequestAllowed.value); }
  ...
  private readonly filterPresets = { ShowEndingSoon: undefined };
  ...
  constructor(
    ...
    this.displayedColumns = [
      entitySchema.Columns.DisplayOrg,
      {
        ColumnName: 'badges',
        Type: ValType.String
      },
      entitySchema.Columns.UiOrderState,
      entitySchema.Columns.OrderDate,
      {
        ColumnName: 'viewDetailsButton',
        Type: ValType.String
      }
    ];
    Object.keys(this.filterPresets).forEach(filterName =>
      this.filterPresets[filterName] = this.activatedRoute.snapshot.queryParamMap.get(filterName)
    );
  ...
  private reorder(request: ItshopRequest, requests: ItshopRequest[]): ItshopRequest[] {
    const parent = request.UID_PersonWantsOrgParent.value ?
      requests.find(item => this.getUid(item) === request.UID_PersonWantsOrgParent.value) : undefined;
