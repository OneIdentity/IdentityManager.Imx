...
export class AppService {
  ...
  public async init(): Promise<void> {
    ...
    this.captchaService.captchaImageUrl = 'opsupport/captchaimage';
  }
  ...
  private async loadModules(appName: string): Promise<void> {
    ...
    for (const plugin of appInfo.PlugIns) {
      if (!plugin.Name || !plugin.Container) {
        this.logger.debug(this, `❌ Malformed plugin: ${plugin.Container}`);
        continue;
      }
      ...
      try {
        ...
        await import(`html/qer-app-operationssupport/${plugin.Container}/fesm2022/${plugin.Container}.mjs`)
          .then((m) => {
            if (plugin.Name) {
